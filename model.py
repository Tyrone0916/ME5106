...
class LaserProcessingNet(nn.Module):
    def __init__(self, spl_result, rms_result, power_speed):
        super(LaserProcessingNet, self).__init__()
        self.spl_result = spl_result
        self.rms_result = rms_result
    ...
    def forward(self, x):
        # 将输入特征（spl_result, rms_result, power_speed）合并成一个向量
        # 假设spl_result和rms_result是标量，power_speed是包含激光功率和扫描速度的向量
        # x = torch.cat((self.spl_result, self.rms_result, self.power_speed), dim=1)  # 合并成一个向量
